...
Copied from qdb_alt/monaqasat/monaqasat/normalize.py on 24 Sep 2026, with one
addition: cr_root understands the QFC/00326 form that the ICV portal uses and
Monaqasat never shows. The two copies are deliberately separate until a second
consumer proves what they actually share; if you change name handling here,
change it there too, or move the file to a shared package and delete both.
...
_QFC = re.compile(r"^QFC[\s/\-]*0*([0-9]+)\s*$", re.I)
...
def cr_root(cr) -> str | None:
    ...
    Handles every form a CR number turns up in, on either side of the join:
      ...
      QFC/00326      Qatar Financial Centre     -> QFC326

    The QFC form is why this file differs from the Monaqasat copy. QFC
    registrations are numbered in their own sequence, so QFC/00326 and
    mainland CR 326 are two unrelated companies; stripping the prefix would
    join a borrower to a stranger. They are kept apart by a prefixed key
    rather than by dropping the row, because a QDB customer may well hold a
    QFC registration and the join has to work for them too.
    """
    if cr is None:
        return None
    if isinstance(cr, str):
        m = _QFC.match(cr.strip())
        if m:
            digits = m.group(1).lstrip("0")
            return "QFC" + digits if digits else None
