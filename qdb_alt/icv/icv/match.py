"""Customers -> ICV suppliers.

Copied from qdb_alt/monaqasat/monaqasat/match.py on 24 Sep 2026. The customer
loader, the name index and the four-method ladder are unchanged -- that logic
is tested there and works; only the emitter is different, because the thing
being joined to is a supplier record rather than a company row on a tender.
Keep the two in step, or move the shared part to its own package and delete
both copies.
...
def match_suppliers(
    ...
    *,
    ...
    """Join a customer book to the ICV supplier register.

    One row per (customer, supplier). A borrower with several rows in the
    book -- a changed registration, a subsidiary, a branch -- is matched on
    every one of them and the results are filed under the single customer id,
    so all of its registrations count. Where two book rows reach the same
    supplier, the stronger evidence wins: a CR match beats a name match.
    """
    index = NameIndex(companies)

    def key(cust, comp):
        return (cust["customer_id"], _field(comp, "id"))

    def emit(cust, comp, method, score):
        return {
            "customer_id": cust["customer_id"],
            "customer_name": cust.get("name"),
            "company_id": _field(comp, "id"),
            "method": method,
            "confidence": score,
            "score": score,
            "matched_on": _field(comp, "cr_number") if method == "cr"
                          else _field(comp, "name"),
...
def summarise(matches) -> dict:
    ...
    return {
        ...
        "suppliers matched": len({m["company_id"] for m in matches}),
        "by method": by_method,
